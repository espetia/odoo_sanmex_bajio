...
#                raise Warning("Factura ya existente con la referencia del vendedor %s"%(customer_reference))
        ...
        forma_pago_data = data.get('Comprobante',{}).get('@FormaPago',{})
        forma_pago = self.get_catalogo_forma_pago(forma_pago_data)
        uso_cfdi_data = receptor_data.get('@UsoCFDI')
        uso_cfdi = self.get_catalogo_uso_de_cfdi(uso_cfdi_data)
...
#                raise Warning("Factura ya existente con la referencia del vendedor %s"%(customer_reference))
    ...
    def import_sale_order(self, file_content):
        ...
        #raise UserError(_("Linea de producto" + str(sale_order_exist)))
    ...
    def get_catalogo_forma_pago(self,clave_forma_pago):
        forma_pago_id = self.env['catalogo.forma.pago'].search([('code','=',clave_forma_pago)]).id
        return forma_pago_id
    
    def get_catalogo_uso_de_cfdi(self,clave_uso_cfdi):
        forma_pago_id = self.env['catalogo.uso.cfdi'].search([('code','=',clave_uso_cfdi)]).id
        return forma_pago_id
...
class HrPayslip(models.Model):
    ...
    cetificaso_sat = fields.Char(string=_('Certificao SAT'))
    ...
    version = fields.Char(string=_('Versión'))
...
#********** DEDUCCIONES *********
        ...
        request_params.update({
                'factura': {
                      ...
                      'total': round(self.total_nomina, 2),
                },
                'emisor': {
                      'rfc': self.company_id.rfc,
                      'curp': self.company_id.curp,                      
                      'nombre_fiscal': self.company_id.razonsocial,
                },
                'receptor': {
                ...
                'nomina12Emisor': {
                      ...
                      'Curp': self.company_id.partner_id.curp
                },
                'nomina12Receptor': {
                      ...
                      'SalarioBaseCotApor': round(self.contract_id.sueldo_base_cotizacion, 2),
                      'SalarioDiarioIntegrado': round(self.contract_id.sueldo_diario_integrado, 2),
                },
...
#****** CERTIFICADOS *******
    ...
    def send_nomina(self):
        ...
        payslip_xml = self.env['ir.attachment'].search([('res_model', '=', 'hr.payslip'), ('res_id', '=', self.id),('name', 'ilike', '.xml')])  
        payslip_pdf = self.env['ir.attachment'].search([('res_model', '=', 'hr.payslip'), ('res_id', '=', self.id),('name', 'ilike', '.pdf')])  
        ...
        ctx.update({
            ...
            'default_attachment_ids': [payslip_pdf.id, payslip_xml.id,]
        })
        _logger.error('ctx: %s', ctx)
        return {

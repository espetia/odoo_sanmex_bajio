...
BEGIN
    ...
    inner join account_fiscal afy on afy.id=ap.fiscalyear_id
    ...
    where date_stop < (select date_start from account_period where id=x_period_id)
    ;
    --and ap.fiscalyear_id in (select af1.id from account_fiscal af1 where af1.name in 
    --                (select af2.name from account_fiscal af2 where af2.id=(select ap2.fiscalyear_id from account_period ap2 where ap2.name=_period_name limit 1)))
    --                ;
    ...
    select name into _period_fiscalyear  from account_fiscal where id in (select fiscalyear_id from account_period period where period.id=x_period_id);
    ...
    select  account.id, x_uid::integer create_uid, account.create_date, account.id account_id, account.code account_code, 
        ...
        case 
            when account.sing_1 = 1 then 'Deudora'
            else 'Acreedora'
        end::varchar(10) account_nature,        
        False::boolean from_partner_breakdown, null::varchar(250) partner_name, null::integer partner_id, account.sing_1 account_sign,
    ...
    LOOP
        ...
        0.0::float initial_balance, 0.0::float debit,
        0.0::float credit, 0.0::float balance, 0.0::float ending_balance,
        company.name company_name, _record2.parent_id, account.sing_1 account_sign, 
        case 
            when account.sing_1 = 1 then 'Deudora'
            else 'Acreedora'
    ...
    LOOP
        ...
        set 
            ...
            and line.period_id in (select pp2.id from period_ids2 pp2)
                             ...
                             --   where xperiodo.fiscalyear_id in (select id from account_fiscal where name = _period_fiscalyear)
...
LANGUAGE 'plpgsql';
            ...
            sql = sql1 + sql4
            #params = [x.id for x in self.env['account.monthly_balance'].search([('create_uid', '=', self._uid)])]           
...
$BODY$ LANGUAGE 'plpgsql' ;
            ...
            BEGIN
                select account.sing_1 into _sign from account_account account where account.id=x_account_id;
                ...
                and ap.fiscalyear_id in (select af1.id from account_fiscal af1 where af1.name in 
                                (select af2.name from account_fiscal af2 where af2.id=(select ap2.fiscalyear_id from account_period ap2 where ap2.name=_period_name_start limit 1)));
                ...
                select                 
                    ...
                    from argil_account_move_line line
                        inner join account_journal journal on line.journal_id=journal.id --and 
                        --(CASE WHEN _period_month = 1 and not _period_flag_00 THEN journal.type <> 'situation' ELSE 1=1 END)
                    where --line.parent_state='posted' and
            ...
            sql2 = """
                ...
                account_type.name account_type, account.sing_1 account_sign, account.id account_id, account_type.id account_type_id,
            ...
            if self.output == 'list_view':
                ...
                #_logger.error("Consulta: %s", values)
                return value
            elif self.output == 'pdf':
    ...
    def get_info(self):
        ...
        if self.report_id and self.report_id.id:
            ...
            BEGIN
                ...
                from account_fiscal fiscal where fiscal.id = (select account_period.fiscalyear_id from account_period where account_period.id = $2);
                ...
                from account_period period, account_fiscal fiscalyear,
                ...
                move_line.amount_currency::float amount_currency,
                move_line.currency_id
                from account_move move, account_move_line move_line, account_period period, account_fiscal fiscalyear,
                ...
                move_line.period_id = period.id and --move_line.state='posted' and
                ...
                BEGIN
                    ...
                    select 
                        ...
                        when 1 then 
                            ...
                            and line.period_id in 
                                (select xperiodo.id from account_period xperiodo 
                                where xperiodo.fiscalyear_id= (select fiscalyear.id from account_fiscal fiscalyear where period.fiscalyear_id = fiscalyear.id)
                                and xperiodo.name < period.name 
                                )
                            )
                        end::float,
            ...
            _logger.error("DatosAuxiliar: %s", values)

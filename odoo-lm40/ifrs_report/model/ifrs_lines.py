...
class IfrsLines(models.Model):
    ...
    def _get_sum_detail(self, number_month=None):
        """ Calculates the amount sum of the line type == 'detail'
        @param number_month: periodo a calcular
        """
        fy_obj = self.env['account.fiscal']
        period_obj = self.env['account.period']
        context = self._context.copy()
        cx = self._context.copy()
        res = 0.0
        period = ""
        if not cx.get('fiscalyear'):
            cx['fiscalyear'] = fy_obj.find()
        ...
        if brw.acc_val == 'init':            
            if cx.get('whole_fy', False):
                self._cr.execute("""select ap.id from account_period ap
                                    ...
                                    select fy.id from account_fiscal fy
                                    where fy.name < (select name from account_fiscal fy2
                                    where fy2.id=%s));""" % fy_id)
                xres = self._cr.fetchall()
                cx['periods'] = [x3[0] for x3 in xres]
                cx.pop('fiscalyear')
            else:
                self._cr.execute("""select ap.id from account_period ap
                                    where ap.name2 < (select ap2.name2 from account_period ap2 where ap2.id=%s);""" % cx['period_to'])
                xres = self._cr.fetchall()
                cx['periods'] = [x3[0] for x3 in xres]
                cx.pop('fiscalyear')
            ...
            if cx.get('whole_fy', False):
                self._cr.execute("""select id from account_period where fiscalyear_id in (
                                        select id from account_fiscal where name <= (
                                        select name from account_fiscal where id=%s));""" % fy_id)
                xres = self._cr.fetchall()
                cx['periods'] = [x3[0] for x3 in xres]
                cx.pop('fiscalyear')
            else:
                cx['periods'] = [cx['period_to']]
        else:
            ...
            if cx.get('whole_fy', False):
                self._cr.execute("""select ap.id from account_period ap where 
                                    ...
                                    ap.fiscalyear_id in (
                                        select id from account_fiscal where name <= (
                                        select name from account_fiscal where id=%s));""" % (fy_id,fy_id))
                xres = self._cr.fetchall()
                cx['periods'] = [x3[0] for x3 in xres]
                cx.pop('fiscalyear')
            else:
        ...
        if brw.type == 'detail':
            ...
            brw = self.with_context(cx).browse(self.id)            
            for aa in brw.cons_ids:
                ...
                if aa.user_type_id.name != 'Vista':
                    apuntes = self.env['account.move.line'].search([('account_id.name', '=', aa.name),('move_id.period_id.fiscalyear_id','=', fy_id),('parent_state','=','posted')])
                    apuntes_periodo = self.env['account.move.line'].search([('account_id.name', '=', aa.name),('move_id.period_id','=', period),('parent_state','=','posted')])
                else:    
                    apuntes = self.env['account.move.line'].search([('account_id.parent_id.name','=', aa.name),('move_id.period_id.fiscalyear_id','=', fy_id),('parent_state','=','posted')])
                    apuntes_periodo = self.env['account.move.line'].search([('account_id.parent_id.name', '=', aa.name),('move_id.period_id','=', period),('parent_state','=','posted')])
                #_logger.error('apuntes: %s', apuntes_vista)
    ...
    def _get_grand_total(self, number_month=None, one_per=False, bag=None):
        ...
        brw = self 
        res = self.with_context(cx)._get_sum_total('total_ids', number_month, one_per=one_per, bag=bag)

        if brw.operator in ('subtract', 'condition', 'percent', 'ratio',
                            'product'):
            so = self.with_context(cx)._get_sum_total('operand_ids', number_month, one_per=one_per, bag=bag)
            if brw.operator == 'subtract':
                res -= so
            elif brw.operator == 'condition':
                res = self.with_context(cx)._get_logical_operation(brw, res, so)
    ...
    def _get_constant(self, number_month=None):
        ...
        if not cx.get('period_from', False) and not cx.get('period_to', False):
            if context.get('whole_fy', False):                
                cx['period_from'] = period_obj.find_special_period(cx['fiscalyear'])
            cx['period_to'] = period_obj.search([('fiscalyear_id', '=', cx['fiscalyear'])])[-1]

        if brw.constant_type == 'period_days':
            res = period_obj._get_period_days(cx['period_from'], cx['period_to'])
        elif brw.constant_type == 'fy_periods':
            res = fy_obj._get_fy_periods(cx['fiscalyear'])
        elif brw.constant_type == 'fy_month':
            res = fy_obj._get_fy_month(cx['fiscalyear'], cx['period_to'])
        elif brw.constant_type == 'number_customer':
            res = self.with_context(cx)._get_number_customer_portfolio(cx['fiscalyear'], cx['period_to'])
        return res
    ...
    def _get_amount_value(self, ifrs_line=None, period_info=None, fiscalyear=None, exchange_date=None, currency_wizard=None,
                                ...
                                two=None, one_per=False, bag=None):
        ...
        if ifrs_line.type == 'detail':
            res = self.with_context(context)._get_sum_detail(number_month)
    ...
    def _get_dict_amount_with_operands(self, ifrs_line, period_info=None, fiscalyear=None,
        ...
        for number_month in range(1, 13):
            ...
            bag[ifrs_line.id][field_name] = self._get_amount_value(ifrs_line, period_info, fiscalyear,
                                                exchange_date, currency_wizard, number_month, target_move, pdx,
                                                undefined, two, one_per=one_per, bag=bag) * direction
    ...
    def _get_amount_with_operands(self, ifrs_l, period_info=None, fiscalyear=None,
        ...
        res = self.with_context(context)._get_amount_value(ifrs_l, period_info, fiscalyear, exchange_date,
                                                           currency_wizard, number_month, target_move, pdx, 
    ...
    priority = fields.Integer(string='Prioridad', default=_get_default_sequence, related='sequence',
                                help=('Indica el orden de la línea en el reporte. La secuencia '
                                      'debe se única e irrepetible'))
    name = fields.Char(string='Nombre', size=128, required=True, translate=True,
                        help=('Nombre de la línea en el reporte. Este nombre puede ser traducible en '
    ...
    ifrs_id = fields.Many2one('ifrs.ifrs', string='IFRS',required=False)
    ...
    inv_sign = fields.Boolean(string='Cambiar el signo al Monto', default=False, copy=True)
    ...
    def _get_level(self, lll, tree, level=1):
        ...
        for jjj in set(lll.total_ids + lll.operand_ids):
            ...
            self._get_level(jjj, tree, level + 1)
        return True

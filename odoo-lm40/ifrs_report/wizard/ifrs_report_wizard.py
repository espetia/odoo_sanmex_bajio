...
def get_xls(rec):
    ...
    if rec.columns=='2_columns':
        ...
        l += 1
        for ifrs_l in rec.ifrs_id.get_report_data(rec.id, rec.fiscalyear_id.id, rec.exchange_date, rec.currency_id.id, rec.target_move, rec.period.id, two=True):
        ...
        for ifrs_l in rec.ifrs_id.get_report_data(rec.id, rec.fiscalyear_id.id, rec.exchange_date, rec.currency_id.id, rec.target_move):
            if ifrs_l.get('invisible'):
                continue
            l += 1
...
class IfrsReportWizard(models.TransientModel):
    ...
    def _default_fiscalyear(self):
        return self.env['account.fiscal'].find()
    ...
    fiscalyear_id = fields.Many2one(
        'account.fiscal', string='Año Fiscal',
    ...
    def print_report(self):
        ...
        context['fiscalyear'] = self.fiscalyear_id.id #AC
        if datas['report_type'] == 'all':            
            datas['period'] = False
            context['period'] = False #AC
        else:
            datas['period'] = self.period.id
            context['period'] = self.period.id #AC

        context['xls_report'] = False
        if self.report_format == 'preview':
            if self.columns=='12_columns':
                return self.env.ref('ifrs_report.ifrs_landscape_pdf_report_action_html').report_action(self)
            else:
                return self.env.ref('ifrs_report.ifrs_portrait_pdf_report_action_html').report_action(self)
        if self.report_format == 'spreadsheet':
            context['xls_report'] = True
            if self.columns=='12_columns':
                
                html1, html2 = self.env.ref('ifrs_report.ifrs_landscape_pdf_report_action')._render_qweb_html(self.ids)
            else:
                html1, html2 = self.env.ref('ifrs_report.ifrs_portrait_pdf_report_action')._render_qweb_html(self.ids)
        ...
        if self.columns=='12_columns':
            return self.env.ref('ifrs_report.ifrs_landscape_pdf_report_action').report_action(self)
        else:
            return self.env.ref('ifrs_report.ifrs_portrait_pdf_report_action').report_action(self)
